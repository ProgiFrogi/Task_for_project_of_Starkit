...
def make_photo():
    ...
    rcam = cv2.VideoCapture('http://192.168.0.103:4747/video?640x480')
    ...
    while True:
        ...
        if (key == ord('q')):
            ...
            cv2.imwrite("stereocallib/" + str(i) + "_l.jpg", fl)
            cv2.imwrite("stereocallib/" + str(i) + "_r.jpg", fr)
...
def callibration():
    cal = StereoCalibration('./stereocallib')
    cal.camera_model
